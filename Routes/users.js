                ...
                if (result.length > 0) {
                  ...
                  bcrypt.hash(Password, 10, (err, hash) => {
                    if (err) {
                      res
                        .status(200)
                        .json({ message: "Error Password Hashing" });
                    } else {
      ...
      if (bcrypt.compareSync(OldPassword, user[0].Password) === true) {
        if (ConfirmNewPassword !== NewPassword) {
          res
            .status(200)
            .json({
              message: "The Given New Password Don't Match",
              success: false,
            });
        } else {
          bcrypt.hash(NewPassword, 10, (err, newPass) => {
            User.findByIdAndUpdate(user[0]._id, { $set: { Password: newPass } })
              ...
              .then((successUpdate) => {
                res
                  .status(200)
                  .json({ message: "Your Password is Updated", success: true });
              });
          });
        }
      } else {
        res
          .status(200)
          .json({ message: "Your Old Password is Invalid", success: false });
      }
    });
});
